...
class Solver(object):
  ...
  def __init__(self, model):
    ...
    # Channel edgle lens
    edge_lens = assemble(theta_tr('+') * dS + theta_tr * ds).array()
    ...
    def S_rhs1(t, S_n):
      ...
      dsdt = assemble((dS_dt *  theta_tr)('+') * dS).array() / edge_lens
      return dsdt
    ...
    zs = np.zeros(len(Y0))
    # Set up ODE solver
    ode_solver = ode(rhs).set_integrator('dopri5', max_step = 20.0, atol = 5e-7, rtol = 1e-9, nsteps = 1000)
  ...
  def step_ode(self, dt):
    ...
    self.q_c_n = assemble((self.q_c *  self.theta_tr)('+') * dS).array() / self.edge_lens
    self.N_cubed_n = assemble((self.N**3 *  self.theta_tr)('+') * dS).array() / self.edge_lens
    self.dphi_ds_n = assemble((self.dphi_ds * self.theta_tr)('+') * dS).array() / self.edge_lens
    self.dpw_ds_n = assemble((self.dpw_ds * self.theta_tr)('+') * dS).array() / self.edge_lens
